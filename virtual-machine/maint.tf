...
resource "azurerm_network_interface" "vm_nic" {
  name                = "nic-${var.prefix}-${var.postfix}"
...
resource "azurerm_network_security_group" "vm_nsg" {
  name                = "nsg-${var.prefix}-${var.postfix}"
  location            = var.location
  resource_group_name = var.rg_name
...
resource "azurerm_virtual_machine" "syn_vm" {
  name                  = "wvm-${var.prefix}-${var.postfix}"
  location              = var.location
  resource_group_name   = var.rg_name
  ...
  storage_os_disk {
    name              = "disk-${var.prefix}-${var.postfix}"
    caching           = "ReadWrite"
...
resource "azurerm_dev_test_global_vm_shutdown_schedule" "vm_schedule" {
  virtual_machine_id = azurerm_virtual_machine.syn_vm.id
  location           = var.location
  enabled            = true
